...
instance Arbitrary a => Arbitrary (Expression a) where
  ...
  shrink (DownCast a e) = (shrink e) ++ [e] ++ [DownCast na ne | na <- shrink a, ne <- shrink e]
  shrink (UpCast a e) = (shrink e) ++ [e] ++ [DownCast na ne | na <- shrink a, ne <- shrink e]
  shrink (DeadCast a e) = (shrink e) ++ [e] ++ [DownCast na ne | na <- shrink a, ne <- shrink e]
